...
@section('app')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.courses.update',$course->id) }}" method="post" autocomplete="off"
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="active" id="active">
                                ...
                                <option value="0" {{ old('active',$course->active) == 0? 'selected':'' }}>Not Active</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="category_id" id="category_id">
                                ...
                                @foreach($categories as $item)
                                    <option value="{{ $item->id }}" {{ old('category_id',$course->category_id) == $item->id? 'selected':'' }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="level" id="level">
                                <option value="beginner" {{ old('level',$course->level) == 'beginner'? 'selected':'' }}>Beginner
                                </option>
                                <option value="immediate" {{ old('level',$course->level) == 'immediate'? 'selected':'' }}>Immediate
                                </option>
                                <option value="high" {{ old('level',$course->level) == 'high'? 'selected':'' }}>High</option>
                            </select>
